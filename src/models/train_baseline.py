...
TRAIN_PATH = GOLD_DIR / "train_features.csv"
VALIDATION_PATH = GOLD_DIR / "validation_features.csv"
TEST_PATH = GOLD_DIR / "test_features.csv"
...
def check_input_files() -> None:
    ...
    if missing_files:
        ...
        raise FileNotFoundError(
            "Missing feature files. Please run `python src/features/build_features.py` first.\n"
            f"{missing}"
        )
...
def build_markdown_report(metrics: dict) -> str:
    ...
    lines.append("# Baseline Model Results")
    lines.append("")
    ...
    lines.append("")
    lines.append("The baseline uses a temporal split instead of a random split to better simulate a real fraud detection setting.")
    lines.append("")
    ...
    lines.append("- Feature set: numerical transaction features, transaction type one-hot encoding, balance error features and ratio features")
    ...
    lines.append("")
    lines.append("- `reports/baseline_metrics.json`")
    lines.append("- `docs/baseline_results.md`")
    lines.append("- `models/logistic_regression_baseline.joblib`")
    lines.append("- `models/hist_gradient_boosting_baseline.joblib`")
    lines.append("")
...
def main() -> None:
    ...
    print(f"Logistic Regression training completed in {logistic_time:.2f} seconds.")
    ...
    print(f"HistGradientBoosting training completed in {hgb_time:.2f} seconds.")
    ...
    joblib.dump(hgb_model, MODEL_DIR / "hist_gradient_boosting_baseline.joblib")
